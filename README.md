...
- **各地区订阅价格**：切换 ChatGPT Plus / Claude Pro，按价格从低到高列出各地区 App Store 标价、约合美元和人民币、比美区贵/便宜多少（标价为 2026 年 8–9 月第三方统计，只收录多个来源一致的数字），每行可点 ↗ 到 App Store 官方页面核实
...
价格表里只存各地区的当地标价，约合美元和人民币由页面按同一套汇率换算：

- 打开页面时请求 [Miao API](https://api.miao.club/#/api/fx) 的 `GET https://api.miao.club/api/fx/rates?base=USD&symbols=CNY,JPY,PHP,PKR,TRY,HUF,NGN,CAD,EGP`（上游为 ExchangeRate-API，每日更新），结果在浏览器本地缓存 6 小时
- 请求失败（接口未放行跨域、网络不通、在禁止外部请求的预览环境里等）时，使用 `js/app.js` 里的内置参考汇率 `FX_FALLBACK`
- 表格下方会注明当前用的是实时汇率还是内置汇率，以及汇率更新时间

浏览器直接调用接口需要响应头带 `Access-Control-Allow-Origin`。
