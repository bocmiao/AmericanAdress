...
- **各地区订阅价格**：列出 2026 年 8 月 App Store 上 ChatGPT Plus、Claude Pro 最便宜的地区，并链接到 12 个国家/地区的 App Store 官方页面查看实时价格
- 首屏右侧的代码窗口以 JSON 形式实时显示当前地址，可一键换一个或复制；支持深浅色切换（默认跟随系统）、可以生成公寓号（第二行地址）
...
差价只剩每月 $2–4，而这些地区要用当地的礼品卡或支付方式，通过第三方购买一般要加价，差价基本抵消；价格也可能随时调整。美区账户加免税州地址是 $19.99 且不加税，礼品卡好买、App 最全，综合下来更省事。页面里的价格链接直接打开 App Store 官方页面，价格以实时显示为准。
